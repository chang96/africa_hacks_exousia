...
const setLists = function (data) {
    ...
    data.forEach((dat, i)=> {
        ...
        <td><button id=${data.email}_${dat.id} onclick="goto(this.id)" class="btn btn-danger">View Users Details</button></td>
        </td>
    </tr>`
        html+=htm
    })
...
$(document).ready(()=>{
    auth.onAuthStateChanged(async user => {
        ...
        if(user.emailVerified === true || 1== 1){
            db.collection('users').where('classs', '==', 'intermediate').get().then(props=>{
...
const goto = function(id){
    ...
    const dets = id.split('_')
    console.log(dets)
    localStorage.setItem("email", dets[0])
    localStorage.setItem("userId", dets[1])
