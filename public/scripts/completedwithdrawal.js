...
const setLists = function (data) {
    ...
    data.forEach((dat, i)=> {
        ...
        <td>${data.status}</td>
        <td><button id=${dat.id} onclick="resolve(this.id)" class="btn btn-danger">Reverse</button></td>
        </td>
    </tr>`
        html+=htm
    })
...
$(document).ready(()=>{
    auth.onAuthStateChanged(async user => {
        ...
        if(user.emailVerified === true || 1== 1){
            db.collection('withdrawals').where('status', '==', 'resolve').get().then(props=>{
...
const resolve = function(id){
    db.collection('withdrawals').doc(id).update({status: 'pending'}).then(()=>{
        alert("withdrawal reversed")
    })
}