...
const setLists = function (data) {
    ...
    data.forEach((dat, i)=> {
        ...
        <td><button id=${dat.id} onclick="goto(this.id)" class="btn btn-danger">View and Edit Blog</button></td>
        
        <td><button id=${dat.id}i onclick="del(this.id)" class="btn btn-danger">Delete</button></td>
...
$(document).ready(()=>{
    auth.onAuthStateChanged(async user => {
        ...
        if(user.emailVerified === true || 1== 1){
            db.collection('blogImages').get().then(props=>{
...
const goto = function(blogid){
    console.log(2345678)
    localStorage.setItem("blogid", blogid)
    window.location = '/dashboard/editblog.html'
}
