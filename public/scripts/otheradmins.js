...
const List = document.getElementById('adminlogs')
...
$(document).ready(()=>{
    auth.onAuthStateChanged(async user => {
        ...
        if(user.emailVerified === true || 1== 1){
            db.collection('logs').get().then(props=>{
...
const updateadmin =async function(){
    ...
    await auth.createUserWithEmailAndPassword(email, password).then(c => console.log(c.user)).catch(e=> {
        alert(e.message)
    })
