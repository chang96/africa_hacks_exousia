...
const setLists = function (data) {
    ...
    data.forEach((dat, i)=> {
        ...
        <td>${data.topic}</td>
        <td>${data.classs}</td>
        <td><button id=${dat.id} onclick="del(this.id)" class="btn btn-danger">Delete</button></td>
        </td>
    </tr>`
        html+=htm
    })
...
$(document).ready(()=>{
    auth.onAuthStateChanged(async user => {
        ...
        if(user.emailVerified === true || 1== 1){
            db.collection('timetable').get().then(props=>{
...
const del= function(id){
    console.log(id)
    db.collection('timetable').doc(id).delete().then(function(d){
        console.log(d)
        swal('Timetable deleted')
    }).catch(e=> swal(e))
}
