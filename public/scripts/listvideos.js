...
const setLists = function (data) {
    ...
    data.forEach((dat, i)=> {
        ...
        <td><video width='200px' height='200px' controls><source src=${data.url}> </video> </td>
        <td>${data.title}</td>
        <td><button id=${dat.id} onclick="deleteVideo(this.id)" class="btn btn-danger">Delete</button></td>
        </td>
    </tr>`
        html+=htm
    })
...
$(document).ready(()=>{
    auth.onAuthStateChanged(async user => {
        ...
        if(user.emailVerified === true || 1== 1){
            db.collection('videos').get().then(props=>{
